...
class Processor():
    ...
    def __init__(self, arg):
        self.arg = arg     # get parser에서 정의한 파라미터 로드
        self.save_arg()    # 모델별 train.yaml 파일 로드
        self.load_data()   
        self.load_model()
        self.load_optimizer()

    """
    load_data는 저장된 데이터 읽어와서 데이터로더 만드는 함수
    """
    def load_data(self):
        Feeder = import_class(self.arg.feeder) #st_gcn.net.st_gcn =>yaml파일에 정의
        self.data_loader = dict()
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=Feeder(**self.arg.train_feeder_args), #딕셔너리타입 매개변수로 받음
    ...
    """
    load_model은 argument에 model에 있는 클래스 불러들여와서 gpu에 할당하는 작업
        - 저장된 가중치 있으면 불러오기 가능.(argument통해 경로 지정해줘야함)
    """
    def load_model(self):
