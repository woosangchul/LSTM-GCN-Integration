...
from .Long_LSTM_Top import Long_LSTM_Top

default_backbone = [(64, 64, 1), (64, 64, 1), (64, 64, 1), (64, 128,
                                                            1), (128, 128, 1),
                    (128, 128, 1), (128, 256, 1), (256, 256, 1), (256, 256, 1)]
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.long_lstm_top = Long_LSTM_Top(True, backbone_config[-1][1], 100, num_class).to(device="cuda:0")
        # tail
    ...
    def forward(self, x):
        ...
        # model
        # initial x shape : (32, 3, 300, 25)
        # output x shape : (24, 64, 300, 25)
        x = self.gcn0(x)
        #x = self.tcn0(x)

        # last output x shape : (24,256,300,25) (batch_size, channel, T, node)
        for m in self.backbone:
            x = m(x)

        # V pooling
        # output x shape : (24, 256, 300, 1)
        x = F.avg_pool2d(x, kernel_size=(1, V))

        # M pooling
        # output x shape : (12, 256, 300)
        x = x.view(N, M, x.size(1), x.size(2))
        x = x.mean(dim=1)


        x = x.permute(0,2,1).contiguous()
        x = self.long_lstm_top(x)

        '''
        # T pooling
        # output x shape : (12, 256, 1)
        x = F.avg_pool1d(x, kernel_size=x.size()[2])


        # C fcn
        # output x shape : (12, 60, 1)
        x = self.fcn(x)

        # output x shape : (12, 60, 1)
        x = F.avg_pool1d(x, x.size()[2:])

        # output x shape : (12, 60)s
